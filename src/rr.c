...
#ifdef MACOSX
char pathToResourcesDir[PATH_MAX];
char pathToPrefFile[PATH_MAX];
static void initMacOSX(int argc, char *argv[]) {
    char *tmppath;
    FSRef *fsRef;
    strcpy(pathToResourcesDir, argv[0]);
    tmppath = strrchr(pathToResourcesDir, '/');
    if (tmppath != NULL) {
        strcpy(tmppath, RESOURCES_DIR);
    } else {
        fprintf(stderr, "strrchr(pathToResourcesDir, '/') == NULL\n");
        fprintf(stderr, "pathToResourcesDir = %s\n", pathToResourcesDir);
        exit(1);
    }
    fsRef = (FSRef*) malloc(sizeof(FSRef));
    if (fsRef == NULL) {
        fprintf(stderr, "malloc(sizeof(FSRef)) == NULL\n");
        exit(1);
    }
    FSFindFolder(kUserDomain, kPreferencesFolderType, kCreateFolder, fsRef);
    FSRefMakePath(fsRef, pathToPrefFile, PATH_MAX);
    strcat(pathToPrefFile, PREF_FILE);
    free(fsRef);
}
#endif
